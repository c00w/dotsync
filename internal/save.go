...
func (c *collator) Walker(path string, info os.FileInfo, err error) error {
    ...
    if !info.IsDir() && strings.HasSuffix(info.Name(), "rc") {
        *c = append(*c, path)
    }
    return nil
}
...
func UpdateIndex(srcpath, curdir string) {
    index, err := os.OpenFile(filepath.Join(curdir, "INDEX"), os.O_APPEND|os.O_RDWR|os.O_CREATE, 066)
    if os.IsNotExist(err) {
        index, err = os.Create(filepath.Join(curdir, "INDEX"))
    }
    if err != nil {
        log.Fatal(err)
    }
...
func OverWrite(in []string, curdir string) {
    for _, srcpath := range in {
        MoveFile(srcpath, curdir)
        UpdateIndex(srcpath, curdir)
    }
}
